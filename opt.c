...
argparse(int argc, char* argv[])
{
	...
	if(nbricks < ht_size) {
		g_warning("more hash table entries (%d) than bricks (%u)!",
		          ht_size, nbricks);
	}
}
...
static bool debug;
static int ht_size;
static int bricks[3]; /* per-dimension. */
static int n_lod;
static char* requestf; /* where to read requests from */
...
#else
  // just init stuff to some reasonable things
  ht_size = 50;
  bricks[0] = 64;
  bricks[1] = 64;
  bricks[2] = 60;
  n_lod = 6;
  requestf = "smaller.req";
  blockfactor = 4;
  ht_naive = false;
  is_verbose = true;
#endif
